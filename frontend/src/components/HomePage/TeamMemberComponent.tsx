...
export default function TeamMember(props: any) {
	return (
		<div
			className={`Member ${props.Color} transition duration-500 ease-in-out font-pixelify max-w-[1536px] min-[0px]:place-content-center lg:px-4 min-[730px]:flex-row lg:flex sm:mx-5 border-solid border-4 border-black shadow-[2px_4px_0px_0px_#000301] py-6`}
		>
			<div
				className={`${props.order} border-solid border-2 sm:border-4 border-black bg-white min-[0px]:p-1 sm:p-4 md:px-6 flex place-content-center mx-2 ml-[13%] lg:m-auto min-[0px]:w-[35%] sm:w-[25%] md:w-[25%] lg:w-[25%] lg:mx-9`}
			>
				<img
					src={ProfilePic}
					alt="pic"
					className="w-[95%] border-solid border-black border-2 sm:border-4 bg-DefaultColor"
				></img>
			</div>
			<div className="flex-col justify-center w-[75%] lg:p-7 m-auto">
				<h1 className="text-xs sm:text-xl md:text-2xl lg:text-3xl xl:text-5xl my-3 mt-6">{props.name}</h1>
				<Socials TeamLinks={TeamLinks} />
				<p className="text-xs sm:text-base md:text-xl xl:text-2xl my-3"> {props.description} </p>
			</div>
		</div>
	);
}
