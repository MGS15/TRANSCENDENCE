...
const useGet2faState = (prop: useGet2faStateProp) => {
	useEffect(() => {
		try {
			fetch("http://hahach.ddns.net:3001/auth/is2fa", {
				method: "GET",
				credentials: "include",
				headers: {
	...
	useEffect(() => {
		try {
			if (!TwoFa && ref.current.checked  ) {
				...
				fetch("http://hahach.ddns.net:3001/auth/generateQrCode", {
					method: "POST",
					credentials: "include",
					headers: {
	...
	useEffect(() => {
		try {
			if (TwoFa && !ref.current.checked  ) {
				...
				fetch("http://hahach.ddns.net:3001/auth/disable2fa", {
					method: "POST",
					credentials: "include",
					headers: {
						"Content-Type": "application/json",
...
const TwoFaBar = ({ toogle, setToggle }: { toogle: any; setToggle: any }) => {
	...
	const handleSubmit = async (e: React.ChangeEvent<HTMLFormElement>) => {
		...
		try{
		 ...
		 await fetch("http://hahach.ddns.net:3001/auth/checkValidcode", {
				method: "POST",
				credentials: "include",
				headers: {
					"Content-Type": "application/json",
