	...
	return (
		<div className="flex sm:justify-center sm:p-6 sm:gap-x-4 align-center justify-items-center min-[0px]:justify-end mr-2 ml-2 sm:mr-0 sm:ml-0">
			<div className="flex self-center min-[0px]:p-2 lg:p-4 justify-center ring-2 ring-black shadow-[4px_4px_0px_0px_#000301]">
				<h1 className="font-Nova min-[0px]:text-sm sm:text-base md:text-xl lg:text-2xl xl:text-3xl">{props.score}</h1>{" "}
			</div>
		</div>
	);
}

function Player({reverse, name, score}:{reverse: boolean, name: string, score: number}) {
	return (
		<div
			className={`flex justify-between ${reverse ? 'flex-row-reverse' : 'flex-row'} border-2 border-solid border-black md:gap-x-14 sm:p-4 p-2 align-center w-full sm:w-[45%] lg:w-[40%] shadow-[2px_4px_0px_0px_#000301] sm:h-[6.5rem] bg-white`}
		>
			<img
				src={Profil}
...
export default function HistoryMatch(props: any) {
	return (
		<div className="flex justify-evenly flex-col sm:flex-row gap-y-4">
			<Player reverse={false} name={props.player1} score={props.one} />
			<Player reverse={true} name={props.player2} score={props.two} />
		</div>
	);
}
