...
export class AppGateway {
	...
	async handleDisconnect(client) {
		...
		if (!(await this.server.to(identifier).fetchSockets()).length) {
			...
			this.statusnotify.emit("PUSHSTATUS", state.user42, [
				{ nickname: state.nickname, connection_state: state.connection_state },
			]);
		}
	}
	...
	async getPrimarystatus(@GetCurrentUser("user42") identifier: string, @ConnectedSocket() client) {
		...
		status.friendship1?.map((friend) => {
			friend.status === "DEFAULT"
				? allstatus.push({
						nickname: friend.reciever_id.nickname,
						connection_state: friend.reciever_id.connection_state,
				  })
		...
		status.friendship2?.map((friend) => {
			friend.status === "DEFAULT"
				? allstatus.push({
						nickname: friend.initiator_id.nickname,
						connection_state: friend.initiator_id.connection_state,
				  })
	...
	async notifyALL(user: string, status: []) {
		...
		friends.forEach(async (friend) => {
			if ((await this.server.to(friend).fetchSockets()).length) this.server.to(friend).emit("ON_STATUS", status);
		});
	}

	@OnEvent("PUSH")
	async informuser(user42, invite, type) {
		if ((await this.server.to(user42).fetchSockets()).length) this.server.to(user42).emit(type, invite);
	}
