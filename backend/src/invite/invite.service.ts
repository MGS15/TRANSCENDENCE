...
export class InviteService {
	...
	async InviteFriend(user: number, friend: number) {
		const res = await this.prisma.$transaction(async (trx) => {
			...
			if (friendship.length || pending.length) return null;
			return await trx.invites.create({
				data: { type: invitetype.Friend, status: actionstatus.pending, issuer: user, reciever: friend },
				select: {
					id: true,
					type: true,
					created_at: true,
	...
	async RemoveFriend(user: number, friend: number) {
		...
		if (!control) throw new HttpException("if blocked resolve it first, else try  to be friends first", 400);
		return control;
	}
