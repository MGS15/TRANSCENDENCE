...
export class InviteController {
	...
	async Friendinvite(@GetCurrentUserId() user: number, @Query("friend") friend: number, @Res() res) {
		...
		if (!invite) throw new HttpException("Failed inviting", HttpStatus.BAD_REQUEST);
	...
	async FriendAccept(@GetCurrentUserId() user: number, @Query("id") id: number, @Res() res) {
		...
		let invite;
		if (Array.isArray(data)) invite = data[0];
		else invite = data;
		...
		this.events.emit(
			"PUSH",
			invite.reciever_id.user42,
			[{ nickname: invite.issuer_id.nickname, connection_state: invite.issuer_id.connection_state }],
			"ON_STATUS",
		);

		this.events.emit(
			"PUSH",
			invite.issuer_id.user42,
			[{ nickname: invite.reciever_id.nickname, connection_state: invite.reciever_id.connection_state }],
			"ON_STATUS",
		);
		...
		if (Array.isArray(data)) {
			this.events.emit(
				"PUSH",
				invite.reciever_id.user42,
				{ region: "ROOM", action: "JOIN", data: data[1] },
				"ACTION",
			);
